...
class Run():
    ...
    def main(self):
        ...
        # 将指标添加到DataFrame中
        columns_order = ['model_name'] + list(json_result[0].keys())
        metrics_df = pd.DataFrame(json_result, columns=columns_order)
        metrics_df['model_name'] = self.config['model_name']
        # 保存到CSV文件，文件名基于模型名称
        file_dir = './result'
        if not os.path.exists(file_dir):
            os.makedirs(file_dir)
        file_name = f"{file_dir}/{self.config['model_name']}_metrics.csv"
        metrics_df.to_csv(file_name, index=False, float_format='%.3f')